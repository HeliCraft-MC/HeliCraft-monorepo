...
import type { GovernmentForm, IState } from '~/interfaces/state/state.types';
...
export default defineEventHandler(async (event) => {
    ...
    if (hasElections !== undefined)
        parsedHasElections = hasElections === 'true';
    if (allowDualCitezenship !== undefined)
        parsedAllowDualCitizenship = allowDualCitezenship === 'true';
    if (freeEntry !== undefined)
        parsedFreeEntry = freeEntry === 'true';

    const strOrNull = (v: unknown): string | null | undefined => {
        if (v === undefined)
            return undefined; // ключ не трогаем
        if (typeof v !== 'string')
            return null;
        const s = v.trim();
    ...
    if (stateUuid)
        preparedStructure.uuid = stateUuid;

    if (name !== undefined)
        preparedStructure.name = name;
    if (description !== undefined)
        preparedStructure.description = description;

    if (color !== undefined)
        preparedStructure.color_hex = color;

    if (parsedGovForm !== undefined)
        preparedStructure.gov_form = parsedGovForm;
    ...
    if (rulerUuid !== undefined)
        preparedStructure.ruler_uuid = rulerUuid;
