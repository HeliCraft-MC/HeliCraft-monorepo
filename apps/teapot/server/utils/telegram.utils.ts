...
function getTelegramConfig(): TelegramConfig {
  ...
  if (!botToken)
    throw new Error('TELEGRAM_BOT_TOKEN is missing in runtimeConfig')
  if (!defaultChatId)
    throw new Error('TELEGRAM_CHAT_ID is missing in runtimeConfig')
...
export async function sendPhoto(photoUrlOrBuffer: string | Buffer, caption?: string, chatIdOrOpts?: ChatId | SendOptions, threadId?: number): Promise<void> {
  ...
  try {
    ...
    if (target.message_thread_id)
      form.append('message_thread_id', String(target.message_thread_id))
