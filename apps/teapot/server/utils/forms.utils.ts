...
import type {
    Form, Question, CreateFormDto, UpdateFormDto,
    CreateQuestionDto, UpdateQuestionDto, SubmitResponseDto
...
export async function updateForm(id: number, dto: UpdateFormDto): Promise<Form> {
    ...
    if (dto.title !== undefined) {
        if (dto.title.trim().length === 0) throw createError({ statusCode: 400, statusMessage: 'Title cannot be empty' });
        updates.push('title = ?');
